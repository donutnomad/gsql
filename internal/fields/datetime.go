...
var _ clause.Expression = (*DateTime[string])(nil)

// DateTime 日期时间类型表达式，用于 DATETIME 类型字段 (YYYY-MM-DD HH:MM:SS)
...
func NewDateTime[T any](expr clause.Expression) DateTime[T] {
	return DateTime[T]{
...
// Cast 类型转换 (CAST)
func (e DateTime[T]) Cast(targetType string) clause.Expression {
	return e.castExpr(targetType)
}

// CastDate 转换为 DATE 类型 (CAST AS DATE)
func (e DateTime[T]) CastDate() Date[string] {
	return NewDate[string](e.castDateExpr())
}

// CastTime 转换为 TIME 类型 (CAST AS TIME)
func (e DateTime[T]) CastTime() Time[string] {
	return NewTime[string](e.castTimeExpr())
}

// CastChar 转换为字符串 (CAST AS CHAR)
func (e DateTime[T]) CastChar(length ...int) String[string] {
