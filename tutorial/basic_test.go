...
func TestFunc_DateTime(t *testing.T) {
	...
	t.Run("DATEDIFF", func(t *testing.T) {
		...
		date1 := gsql.DateTimeOf[time.Time](gsql.Lit("2020-06-15"))
		date2 := gsql.DateTimeOf[time.Time](gsql.Lit("2019-01-10"))
...
func TestFunc_Numeric(t *testing.T) {
	...
	t.Run("POWER", func(t *testing.T) {
		...
		err := gsql.Select(gsql.IntOf[int](gsql.Lit(2)).Pow(3).As("power_val")).
			From(&p).
			Limit(1).
			First(db, &result)
		if err != nil {
