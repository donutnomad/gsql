...
func TestFunc_DateTime(t *testing.T) {
	...
	t.Run("DATEDIFF", func(t *testing.T) {
		...
		date1 := fields.NewDateTime[time.Time](gsql.Lit("2020-06-15"))
		date2 := fields.NewDateTime[time.Time](gsql.Lit("2019-01-10"))
...
func TestFunc_Numeric(t *testing.T) {
	...
	t.Run("ABS", func(t *testing.T) {
		...
		err := gsql.Select(fields.NewFloat[float64](gsql.Lit(-99.5)).Abs().As("abs_val")).
			From(&p).
			Limit(1).
			First(db, &result)
		if err != nil {
	...
	t.Run("POWER", func(t *testing.T) {
		...
		err := gsql.Select(fields.NewInt[int](gsql.Lit(2)).Pow(3).As("power_val")).
			From(&p).
			Limit(1).
			First(db, &result)
		if err != nil {
	...
	t.Run("SQRT", func(t *testing.T) {
		...
		err := gsql.Select(fields.NewInt[int](gsql.Lit(16)).Sqrt().As("sqrt_val")).
			From(&p).
			Limit(1).
			First(db, &result)
		if err != nil {
